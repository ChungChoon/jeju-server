...
/** @description 나의 거래 내역 (학생 계정)
 * @method GET
 */
router.get('/buylist', async (req, res, next) => {
    ...
    if (!token) {
        ...
        if (decoded === -1) {
            ...
            let select_query = `
                ...
                order by c.apply_idx desc
                `;
            let query_result = await db.queryParamArr(select_query, [decoded.user_idx]);
            ...
            if (!query_result) {
                ...
                res.status(200).json({
                    message: "Success To Get My Buy List",
                    data: query_result
                })
            }
        }
